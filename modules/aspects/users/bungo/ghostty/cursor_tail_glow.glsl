// Cursor tail with glow.
// Base smear/tail geometry from sahaj-b/ghostty-cursor-shaders (cursor_tail.glsl),
// rendered with a CRT-style glow (white-hot core + colored halo + brightness boost)
// inspired by sarphiv's in-game CRT cursor shader.
...
const float THRESHOLD_MIN_DISTANCE = 0.09; // min distance to show trail (units of cursor width)
const float BLUR = 2.0; // blur size in pixels (for antialiasing) — also the crisp body edge

// --- GLOW CONFIGURATION ---
const float GLOW_SIZE = 8.0;      // halo radius in pixels
const float GLOW_STRENGTH = 0.35; // peak opacity of the halo [0,1]
const float GLOW_DECAY = 5.0;     // how fast the glow fades after a move (higher = quicker) — glow only shows while moving
...
// Convert a pixel radius into the shader's normalized distance units.
float pxToUnits(float px) {
   return normalize(vec2(px, px), 0.).x;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
   ...
   if (lineLength > minDist && progress < 1.0 && iFocus > 0.5) {
      ...
      // -- FINAL SELECTING AND DRAWING (tail + subtle glow) --
      float sdfTrail = mix(sdfTrail_diag, sdfTrail_rect, isStraightMove);

      // Glow uses the cursor's own color (linearized) — no override, no boost.
      vec4 trail = vec4(sRGBToLinear(iCurrentCursorColor.rgb), iCurrentCursorColor.a);

      // Soft halo around the smear, only visible right after a move — decays over time
      // like the CRT cursor. (Needs iTime to keep advancing: custom-shader-animation = always.)
      float glowFade = clamp(exp(-(iTime - iTimeCursorChange) / DURATION * GLOW_DECAY), 0.0, 1.0);
      float edgeGlow = pxToUnits(GLOW_SIZE);
      float haloAlpha = (1.0 - smoothstep(0.0, edgeGlow, max(sdfTrail, 0.0))) * GLOW_STRENGTH * glowFade;
      newColor = mix(newColor, trail, haloAlpha);

      // Solid smear body (same as the plain tail).
      float trailAlpha = 1.0 - smoothstep(0.0, pxToUnits(BLUR), sdfTrail);
      newColor = mix(newColor, trail, trailAlpha);

      // punch hole so the actual cursor is drawn on top
